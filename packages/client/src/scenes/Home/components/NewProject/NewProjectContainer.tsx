...
interface Props extends WithStyles<typeof styles> {
  ...
  onCancel(video: Video | undefined): EmptyAction;
  onNewTag(name: string): AsyncAction<TagData, string>;
}

const mapStateToProps = (state: AppState) => ({
  aVideo: state.home.video,
  tags: state.tags,
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  onCancel: (video: Video) => dispatch(discardNewVideo(video.player, video.id)),
  onSubmit: (project: ProjectCreateData) =>
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(
  withI18n()(class extends React.Component<Props & WithI18n, State> {
    ...
    render() {
      ...
      if (aVideo && user && user.role !== 'Student') {
        return (
          <Dialog
            open={true}
            fullWidth={true}
            onClose={() => onCancel(this.props.aVideo)}
            scroll="body"
          >
            <DialogHeader
            ...
            >
              <div
                ...
                style={{
                  backgroundImage: `url(${aVideo.thumbnailUrl})`,
                }}
              >
            ...
            <DialogActions style={{ textAlign: 'center' }}>
              <Button
                onClick={() => onCancel(this.props.aVideo)}
                color="secondary"
                variant="contained"
              >
                {t('cancelAction')}
              ...
              <Button
                onClick={() => onSubmit({
                  ...project, 
                  video: {title: aVideo.title, player: aVideo.player, path: aVideo.id} 
                })}
                color="primary"
                variant="contained"
              >
