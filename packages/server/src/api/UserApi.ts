...
router.post("/signup", (req, res, next) => {
  ...
  if (!result.success) {
    console.error(
      `Failed user signup with data ${payload}: bad request:`,
      result
    );
    return res.status(400).json(result);
  }
  return authenticate(SigninStrategy.TEACHER_SIGNUP, (error) => {
    if (error) {
      ...
      console.log(
        `New signup from teacher with email ${payload.email}`,
        result
      );
      return res.status(201).json(result);
    }
...
router.post("/login", (req, res, next) => {
  ...
  if (!result.success) {
    console.error(
      `Failed user login with data ${payload}: bad request:`,
      result
    );
    return res.status(400).json(result);
  }
      ...
      if (!user) {
        console.error(
          `Failed to confirm signup: user` +
            ` with login ${payload.login} not found`
        );
        return res.status(401).json({
          success: false,
        ...
        if (compareCodes(user.code, payload.code)) {
          return UserStore.confirmByEmailOrUsername(payload.login)
            .then(() => res.status(200).json(result))
            .catch((error: Error) => {
              console.error(
                `Failed to confirm signup for user` +
                  ` with login ${payload.login}:`,
                error
              );
              return res.status(500).send();
            });
          ...
          console.error(
            `Failed to confirm signup for user with login` +
    ...
    .then((user?: TeacherServerRecord) => {
      if (!user) {
        ...
        if (compareCodes(user.code, payload.code)) {
          ...
          console.error(
            `Failed to confirm password reset for user with email ${payload.email}:` +
...
router.get("/me", isLoggedIn, (req, res) => {
  if (req.user) {
    return res.status(200).json({
      teacher: {
        username: (req.user as UserRecord).username,
