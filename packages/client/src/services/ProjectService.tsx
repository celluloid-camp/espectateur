import { ProjectCreateData, ProjectShareData, ProjectUpdateData, VideoUpdateData } from '@celluloid/types';
...
export default class Projects {
  ...
  static joinProject(shareCode?: string) {
    const headers = {
      'Accepts': 'application/json',
      'Content-type': 'application/json'
    };
    return fetch(`/api/projects/join/${shareCode}`, {
      method: 'POST',
      headers: new Headers(headers),
      credentials: 'include'
    })
    .then(response => {
      if (response.status === 201) {
        ...
        throw new Error(Constants.ERR_NOT_LOGGED_IN);
      } else if (response.status === 500 || !shareCode) {
        throw new Error(t('home.invalidShareCode'));
      } else if (response.status === 503) {
        throw new Error(t('home.invalidUserShared'));
      } else {
        throw new Error(Constants.ERR_UNAVAILABLE);
      }
    });
